...
int count = 0;
	...
	double scale = 2.5; 							//Proportional gain
	...
	int quadrantfour = 0;
	int centred = 0;
	double wallscale = 9.5;
	double wallerror = 0;
	//Proportional stuff
	double derivativeConstant = 340;
	double derivative = 0;
	
	double gain = derivative+wallscale; 
  ...
  init();
	...
	while(count < 8000)
	{
		...
		while (quadrantfour == 1)
		{
			...
			derivative = ((wallscale - centred )/0.1)*derivativeConstant;
			
			/*
			v_left =  35 + (int)(wallerror);  //base motor speed is 60 which should be approx 1.17V
			v_right = 35 - (int)(wallerror);
			* */
			
			v_left =  35 + (int)(gain);  //base motor speed is 60 which should be approx 1.17V
			v_right = 35 - (int)(gain);
		...
		for ( int i = 0 ; i < 32 ;i++)
		{
			...
			wh[i] = 0;
			if ( pix[i] > 110)// get pixel values and  decide if pixels are white
			{
				wh[i] = 1;
			}
		}
		...
		else if (nwp>25)
		{
			...
			else if (tcount == 1 )
			{
				...
				sleep1(0,400000);
				scale = 3.2;
				printf("I'm a good robot and made it past the first dead end\n");
				}
